...
def get_session_timeout():
    """Get session timeout duration in seconds"""
    from config import Config
    return Config.PERMANENT_SESSION_LIFETIME
...
def is_session_expired(session_created_at):
    ...
    timeout = timedelta(seconds=Config.PERMANENT_SESSION_LIFETIME)
...
def can_approve_raawa(user, raawa):
    ...
    approver = SupabaseDB().get_approver_by_user_id(user.id)
    if not approver:
        return False
